...
ORDER BY idea_id
    ...
    def ideas_with_ids_in(self, ids):
        sql = """
SELECT id AS idea_id, ({index_column}) AS content, coalesce(parent_projects.parent_project_id, parent_id) AS project_id
FROM pages
  LEFT OUTER JOIN LATERAL (
...
ORDER BY idea_id
        ...
        sql = sql.format(index_column=" ||  ' ' || ".join(index_column), idea_ids = ",".join([str(i) for i in ids]))
    ...
    def ideas_ids_for_last_hours(self, hours):
        min_created_time = datetime.datetime.now() - datetime.timedelta(hours=hours)
        min_created_time_str = min_created_time.strftime("%Y-%m-%d %H:%M")
        sql = """
SELECT id FROM pages WHERE type IN ('Pim::Idea', 'Pim::VndIdea', 'Pim::BestPracticeIdea') AND created_at > timestamp '{min_created}'
        """

        sql = sql.format(min_created = min_created_time_str)
        sql = sqlalchemy.text(sql)
        ideas = pd.read_sql_query(sql, app_env.db_engine(), params={}, index_col='id')

        return set(ideas.index)

    def ideas_with_tokens(self, ids = None):
